<template>
	<view class="nav">
		<!-- 状态栏 status-->
		<view :style="'height:'+statusH + 'rpx;'+statusBackground"></view>
			<!-- 首页导航栏 -->
			<view class="home-nav" :style="homeNavStyle" v-if="isHome">
				...
				-->
				<navigator url="../../pages/search/index" class="searchContainer" :style="navigatorStyle">
...
<script setup>
	...
	const setNavStyle = () => {
		...
		// 如果当前机型为IOS，则用对应的值
		const isIOS = osName?.indexOf('ios') > -1; // 找到则为true，否则为false
		if (!isIOS) { // 安卓
			containerH.value = 96
		} else { // 苹果
			containerH.value = 88
